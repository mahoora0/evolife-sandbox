...
from config import (
    ...
    INITIAL_HERBIVORES, INITIAL_CARNIVORES, SEASON_DURATION, SEASONS,
    SEASON_MODIFIERS, GRASS_SPAWN_RATE
)
...
class World:
    ...
    def spawn_grass_regional(self, energy_val):
        ...
        if roll < 0.60:
            ...
            x = random.uniform(20, self.width - 20)
            y = random.uniform(20, self.height - 20)
            # Ensure it's not strictly Forest/Desert, or just let Plains overlap
        else:
    ...
    def update_spatial_grid(self):
        ...
        for g in self.grasses:
            if g.alive: register(g, "grass")
        for h in self.herbivores:
            if h.alive: register(h, "herbivore")
        for c in self.carnivores:
            if c.alive: register(c, "carnivore")

    def find_nearest_in_grid(self, creature, target_type):
        """
        Uses spatial grid partition to query nearby entities,
        greatly reducing search operations from O(N) to O(1) in dense areas.
        """
                ...
                if cell_key in self.grid:
                    ...
                    for entity in entities:
                        if entity is creature or not entity.alive:
                            continue
                        dist = math.hypot(entity.x - creature.x, entity.y - creature.y)
                        if dist < min_dist:
                            min_dist = dist
                            nearest = entity
    ...
    def update(self, dt=1.0/60.0, sim_speed=1):
        ...
        for _ in range(sim_speed):
            ...
            base_grass_energy = mods["grass_energy_multiplier"] * 35.0
            ...
            # 3. Update spatial grid for correct local querying
            self.update_spatial_grid()
            ...
            for h in self.herbivores:
                ...
                if h.alive and h.energy > h.reproduction_threshold:
                    child = h.reproduce()
            ...
            for c in self.carnivores:
                ...
                # Check for reproduction
                if c.alive and c.energy > c.reproduction_threshold:
                    child = c.reproduce()
    ...
    def add_grass_at(self, x, y):
        if len(self.grasses) < MAX_GRASS:
            ...
            base_grass_energy = self.get_season_mods()["grass_energy_multiplier"] * 35.0
