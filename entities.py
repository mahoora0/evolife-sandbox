...
class Creature:
    ...
    def steer_towards(self, tx, ty):
        ...
        # Clamp to turn_rate
        steer = max(-self.turn_rate, min(self.turn_rate, angle_diff))
        self.angle += steer
        # Keep angle in normal range
    ...
    def avoid_boundaries(self):
        ...
        if self.x < margin or self.x > config.WORLD_WIDTH - margin or \
           self.y < margin or self.y > config.WORLD_HEIGHT - margin:
            near_border = True
    ...
    def move(self, speed_multiplier=1.0):
        ...
        self.x = max(10.0, min(config.WORLD_WIDTH - 10.0, self.x))
        self.y = max(10.0, min(config.WORLD_HEIGHT - 10.0, self.y))
...
class Herbivore(Creature):
    ...
    def update(self, world, metabolism_multiplier=1.0, speed_multiplier=1.0):
        ...
        if nearest_predator and pred_dist < self.vision:
            ...
            away_x = self.x + dx * (50 * self.fear)
            away_y = self.y + dy * (50 * self.fear)
            self.steer_towards(away_x, away_y)
    ...
    def reproduce(self):
        ...
        cx = self.x + math.cos(offset_angle) * (self.size * 2)
        cy = self.y + math.sin(offset_angle) * (self.size * 2)
...
class Carnivore(Creature):
    ...
    def update(self, world, metabolism_multiplier=1.0, speed_multiplier=1.0):
        ...
        if nearest_prey and prey_dist < self.vision:
            ...
            # Aggression gene affects steering aggressiveness
            # (In this rule system, we track directly, but aggression can slightly modify steering speed or energy cost)
            self.steer_towards(nearest_prey.x, nearest_prey.y)
            
            # Check attack collision
            if prey_dist < (self.size + nearest_prey.size):
                # Kill and eat prey
                nearest_prey.alive = False
                # Carnivore gains energy proportional to the prey's size (large preys are meatier!)
                earned_energy = 40.0 + nearest_prey.size * 6.0
